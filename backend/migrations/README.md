...
当前迁移为 `0001–0007`。`0007_worker_leases.sql` 扩展 execution Job 并新增 Run Attempt、租约代次、到期索引、FORCE RLS 与延迟 lease/attempt 一致性约束；`0006_start_run_plan.sql` 提供 StartRun 的本地计划事实；`0005_coordinated_cancellation.sql` 支持已受理但仍处于原始 blocked 状态任务的取消与原周期预留释放。应用既有库迁移仍由操作员显式执行，不在 API/Worker 启动时自动执行。
...
Worker 控制角色使用 `pnpm db:grant-worker --role mender_worker`。它只获得 execution.jobs／run_attempts 的必要读写，以及 run_admissions 中 workspace/run/deployment_revision 三列读取；不能读取 canonical arguments、机器身份、预算/价格、Connection 或普通 Run 数据，也不能修改 Run。Worker startup 会再次验证角色，不接受 owner/runtime/admission/cancellation 角色替代。完整边界见 [Worker 租约记录](../../docs/engineering/2026-09-10-worker-leases.md)。
