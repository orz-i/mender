...
func GrantCallbackIngestor(ctx context.Context, pool *pgxpool.Pool, role string) error {
	...
	for _, sql := range []string{
		...
		"GRANT SELECT ON mender_meta.schema_migrations,execution.runs,execution.jobs,execution.run_attempts,execution.provider_observations,execution.provider_cancel_intents,execution.artifacts,execution.provider_callback_inbox,execution.agent_input_requests TO " + id,
		"GRANT INSERT ON execution.provider_callback_inbox,execution.provider_observations,execution.run_events,execution.settlement_jobs,execution.artifacts,execution.agent_input_requests TO " + id,
