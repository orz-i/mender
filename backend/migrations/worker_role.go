...
// GrantWorker provisions only the local execution-control role. It cannot read
// admission arguments/identity/commerce/connection secrets and cannot mutate Runs.
func GrantWorker(ctx context.Context, pool *pgxpool.Pool, role string) error {
	...
	for _, sql := range []string{
		...
		"GRANT SELECT ON mender_meta.schema_migrations,execution.jobs,execution.run_attempts TO " + id,
		...
		"GRANT UPDATE (state,lease_until,finished_at) ON execution.run_attempts TO " + id,
	} {
