...
func GrantDangerousOperationManager(ctx context.Context, pool *pgxpool.Pool, role string) error {
	...
	for _, sql := range []string{
		...
		"GRANT EXECUTE ON FUNCTION governance.submit_dangerous_operation(text,text,text,text,text,text,text,text,text,jsonb,text,bigint,text,text,timestamptz,timestamptz) TO " + id,
