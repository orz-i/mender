...
当前执行链已经具备 activation、`SKIP LOCKED` lease、heartbeat、durable submission intent、HTTP Executor seam 和受审 Supervisor。供应商 accepted 后 `Job=provider_waiting` 并立即释放本地 Worker lease；Attempt 同时保存实际 `provider_id + provider_request_id`。Provider Reconciler 只选择具有该完整路由证据的 submitted/unknown Attempt，通过 Supply public status port 查询并把结果写成 append-only `ProviderObservation`；terminal observation 与 Run terminal/Job finished 由数据库延迟约束原子绑定。默认 `cmd/worker` 仍不装配 production provider status reader；Provider cancel、费用结算与 Outbox 外部投递仍属后续切片。
