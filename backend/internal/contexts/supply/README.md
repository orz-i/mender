...
- 候选公开合同：ProviderProfile、DeploymentDescriptor；当前已发布 Executor、ProviderStatusReader 与 ProviderCanceler 运行时合同
...
HTTP outbound adapter 已实现默认 deny 的精确 host allowlist、DNS/IP 校验与固定拨号、redirect/代理禁用、请求响应上限、timeout、idempotency/auth header 注入以及严格 accepted response 解析。Supply public 另发布 ProviderStatusReader 与 ProviderCanceler；Execution 只能经 outbound adapter 按 durable `provider_id` 精确选择对应 capability，不允许 fallback 猜测。当前仍没有 production status/cancel 网络 adapter；本轮取消测试仅使用内存 fake provider。见 [HTTP Executor 记录](../../../../docs/engineering/2026-09-10-http-executor.md)、[Provider Reconciliation](../../../../docs/engineering/2026-09-10-provider-reconciliation.md)与 [Provider Cancellation](../../../../docs/engineering/2026-09-10-provider-cancellation.md)。
