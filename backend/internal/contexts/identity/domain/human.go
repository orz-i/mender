...
func (m WorkspaceMembership) Allows(action string) bool {
	...
	switch action {
	case "workspace:read", "connection:read", "run:read", "usage:read", "catalog:read", "publisher:read", "release:read":
		return true
	case "connection:manage":
		return m.Role == RoleOwner || m.Role == RoleAdmin
	case "catalog:review", "catalog:audit", "catalog:policy", "publisher:review", "release:manage", "execution:governance":
		return m.Role == RoleOwner || m.Role == RoleAdmin
