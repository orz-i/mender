...
func scanRunStartDelegation(row pgx.Row) (domain.RunStartDelegation, error) {
	...
	err := row.Scan(&d.ID, &d.Digest, &d.WorkspaceID, &d.UserID, &d.ToolsetVersionID, &d.ToolID, &d.ToolVersion, &d.ToolVersionID, &d.ConnectionID, &d.Currency, &d.MaxChargeMicro, &d.IdempotencyKey, &d.CreatedAt, &d.ExpiresAt, &d.RevokedAt, &d.UserDisabled, &d.MembershipRole, &d.MembershipDisabled, &d.WorkspaceDisabled)
