...
func DangerousOperationManagerRole(ctx context.Context, pool *pgxpool.Pool) error {
	...
	err := pool.QueryRow(ctx, `SELECT
	 ...
	 AND has_function_privilege(current_user,'governance.submit_dangerous_operation(text,text,text,text,text,text,text,text,text,jsonb,text,bigint,text,text,timestamptz,timestamptz)','EXECUTE')
