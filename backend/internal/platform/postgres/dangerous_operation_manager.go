...
func DangerousOperationManagerRole(ctx context.Context, pool *pgxpool.Pool) error {
	...
	err := pool.QueryRow(ctx, `SELECT
	 ...
	 AND NOT has_table_privilege(current_user,'governance.jit_support_grants','INSERT,UPDATE,DELETE,TRUNCATE')`).Scan(&grants)
	if err != nil || !grants {
		return errors.New("dangerous-operation-manager table grants are invalid")
	}
	err = pool.QueryRow(ctx, `SELECT
	 ...
	 AND has_function_privilege(current_user,'governance.revoke_jit_support(text,text,text,timestamptz,text)','EXECUTE')`).Scan(&grants)
	if err != nil || !grants {
		return errors.New("dangerous-operation-manager function grants are invalid")
	}
	err = pool.QueryRow(ctx, `SELECT
	 NOT has_function_privilege(current_user,'governance.submit_dangerous_operation(text,text,text,text,text,text,text,text,text,jsonb,text,bigint,text,text,timestamptz,timestamptz)','EXECUTE')
	...
	if err != nil || !grants {
		return errors.New("dangerous-operation-manager isolation grants are invalid")
	}
	var rls int
