...
func PublisherManagerRole(ctx context.Context, pool *pgxpool.Pool) error {
	...
	err := pool.QueryRow(ctx, `SELECT
	 ...
	 AND has_schema_privilege(current_user,'governance','USAGE')
	 AND has_function_privilege(current_user,'governance.submit_plugin_publication(text,text,text,text,text,timestamptz,timestamptz)','EXECUTE')
	 AND has_function_privilege(current_user,'governance.publish_approved_plugin(text,text,text,text,timestamptz)','EXECUTE')
	 AND NOT has_table_privilege(current_user,'governance.plugin_publication_approvals','SELECT,INSERT,UPDATE,DELETE,TRUNCATE')
	 AND NOT has_table_privilege(current_user,'governance.plugin_publication_audit_events','SELECT,INSERT,UPDATE,DELETE,TRUNCATE')
	 ...
	 AND NOT has_function_privilege(current_user,'governance.approve_plugin_publication(text,text,text,timestamptz,text)','EXECUTE')
	 AND NOT has_function_privilege(current_user,'governance.reject_plugin_publication(text,text,text,timestamptz,text)','EXECUTE')
