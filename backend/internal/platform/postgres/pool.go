...
func RuntimeRole(ctx context.Context, pool *pgxpool.Pool) error {
	...
	if err = pool.QueryRow(ctx, `SELECT has_table_privilege(current_user,'catalog.tool_versions','SELECT')
	 ...
	 AND has_column_privilege(current_user,'connections.connection_grants','subject_id','SELECT')
	 AND has_column_privilege(current_user,'commerce.budget_periods','ends_at','SELECT')
