...
func CatalogManagerRole(ctx context.Context, pool *pgxpool.Pool) error {
	...
	err := pool.QueryRow(ctx, `SELECT
	 ...
	 AND has_table_privilege(current_user,'governance.catalog_publication_approvals','SELECT')
	 ...
	 AND has_function_privilege(current_user,'governance.submit_catalog_publication(text,text,text,text,text,timestamptz,timestamptz)','EXECUTE')
	 AND has_function_privilege(current_user,'governance.submit_catalog_publication_with_policy(text,text,text,text,text,timestamptz,timestamptz)','EXECUTE')
	 AND NOT has_function_privilege(current_user,'governance.evaluate_catalog_publication_policy(text,text,text,timestamptz)','EXECUTE')
