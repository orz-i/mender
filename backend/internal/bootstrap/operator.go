...
func RunOperator(ctx context.Context, args []string, getenv func(string) string, out, errOut io.Writer) error {
	if len(args) == 0 {
		return errors.New("operator requires migrate, grant-runtime, grant-browser-session, grant-connection-manager, grant-catalog-manager, grant-governance-reviewer, grant-governance-policy-manager, grant-governance-execution-confirmer, grant-commerce-observer, grant-admission, grant-cancellation, grant-worker, grant-executor, grant-mcp-connector, grant-reconciler, grant-callback-ingestor, grant-settlement, provision-human, issue-key or revoke-key")
	}
	command := args[0]
	if command != "migrate" && command != "grant-runtime" && command != "grant-browser-session" && command != "grant-connection-manager" && command != "grant-catalog-manager" && command != "grant-governance-reviewer" && command != "grant-governance-policy-manager" && command != "grant-governance-execution-confirmer" && command != "grant-commerce-observer" && command != "grant-admission" && command != "grant-cancellation" && command != "grant-worker" && command != "grant-executor" && command != "grant-mcp-connector" && command != "grant-reconciler" && command != "grant-callback-ingestor" && command != "grant-settlement" && command != "provision-human" && command != "issue-key" && command != "revoke-key" {
