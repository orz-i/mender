...
func RunOperator(ctx context.Context, args []string, getenv func(string) string, out, errOut io.Writer) error {
	if len(args) == 0 {
		return errors.New("operator requires migrate, grant-runtime, grant-browser-session, grant-connection-manager, grant-oauth-refresher, grant-catalog-manager, grant-publisher-manager, grant-release-manager, grant-dangerous-operation-manager, grant-support-reader, grant-platform-admin-manager, grant-billing-manager, grant-governance-reviewer, grant-governance-policy-manager, grant-governance-execution-confirmer, grant-commerce-observer, grant-admission, grant-cancellation, grant-worker, grant-executor, grant-mcp-connector, grant-reconciler, grant-callback-ingestor, grant-callback-observer, grant-settlement, grant-artifact-materializer, grant-artifact-object-reader, provision-human, provision-platform-staff, issue-key or revoke-key")
	}
	command := args[0]
	if command != "migrate" && command != "grant-runtime" && command != "grant-browser-session" && command != "grant-connection-manager" && command != "grant-oauth-refresher" && command != "grant-catalog-manager" && command != "grant-publisher-manager" && command != "grant-release-manager" && command != "grant-dangerous-operation-manager" && command != "grant-support-reader" && command != "grant-platform-admin-manager" && command != "grant-billing-manager" && command != "grant-governance-reviewer" && command != "grant-governance-policy-manager" && command != "grant-governance-execution-confirmer" && command != "grant-commerce-observer" && command != "grant-admission" && command != "grant-cancellation" && command != "grant-worker" && command != "grant-executor" && command != "grant-mcp-connector" && command != "grant-reconciler" && command != "grant-callback-ingestor" && command != "grant-callback-observer" && command != "grant-settlement" && command != "grant-artifact-materializer" && command != "grant-artifact-object-reader" && command != "provision-human" && command != "provision-platform-staff" && command != "issue-key" && command != "revoke-key" {
