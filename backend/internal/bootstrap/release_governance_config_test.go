...
func TestReleaseGovernanceConfigRequiresOIDCAndSeparateRole(t *testing.T) {
	base := map[string]string{
		...
		"MENDER_ADMIN_RELEASE_GOVERNANCE_ENABLED": "true",
		"MENDER_RELEASE_MANAGER_DATABASE_URL":     "postgres://release-manager-not-connected",
	}
	cfg, err := LoadAPIConfig(func(key string) string { return base[key] })
	if err != nil || !cfg.AdminReleaseGovernanceEnabled || cfg.ReleaseManagerDatabaseURL == "" {
		t.Fatal(cfg, err)
	}
	for _, tc := range []struct{ key, value string }{
		{"MENDER_ADMIN_RELEASE_GOVERNANCE_ENABLED", "yes"},
		{"MENDER_CONSOLE_OIDC_ENABLED", "false"},
		{"MENDER_RELEASE_MANAGER_DATABASE_URL", ""},
	} {
		...
		if _, err = LoadAPIConfig(func(key string) string { return copy[key] }); err == nil {
			t.Fatal("invalid Release Governance config accepted", tc.key, tc.value)
		}
	}
}
