...
func TestDangerousOperationConfigRequiresOIDCAndSeparateRole(t *testing.T) {
	...
	if err != nil || !cfg.AdminDangerousOperationEnabled || cfg.DangerousOperationManagerDatabaseURL == "" {
		t.Fatal(cfg, err)
	}
	for _, tc := range []struct{ key, value string }{
		{"MENDER_ADMIN_DANGEROUS_OPERATION_ENABLED", "yes"},
		{"MENDER_CONSOLE_OIDC_ENABLED", "false"},
		{"MENDER_DANGEROUS_OPERATION_MANAGER_DATABASE_URL", ""},
	} {
		...
		if _, err = LoadAPIConfig(func(key string) string { return copy[key] }); err == nil {
			t.Fatal("invalid Dangerous Operation config accepted", tc.key, tc.value)
		}
	}
}
