...
func TestConsoleLaunchDiscoveryRequiresOIDCAndIsExplicit(t *testing.T) {
	...
	if err != nil || !cfg.ConsoleLaunchDiscoveryEnabled {
		t.Fatal(cfg, err)
	}
	for _, change := range []map[string]string{
		{"MENDER_CONSOLE_LAUNCH_DISCOVERY_ENABLED": "yes"},
		...
		if err == nil {
			t.Fatal("unsafe launch discovery configuration accepted", change)
		}
	}
}
