...
function build(output){
 ...
 for(const name of ['api','worker','operator','provision','healthcheck','mender','localidp','localprovider']){
...
function preflight(input,signed,keyFile){
 ...
 for(const name of ['api-console','api-admin','worker','provision','operator',...(d.config.local_oidc_fixture?['local-idp']:[]),...(d.config.local_demo_fixture?['local-provider']:[])])if(c.services[name]?.image!==release.runtime_image)throw Error('Compose runtime image does not match the verified release');
 ...
 if(d.config.local_demo_fixture){const s=c.services['local-provider'];if(!s||s.read_only!==true||s.privileged||s.network_mode!=='service:worker'||s.user==='0:0'||!s.cap_drop.includes('ALL')||!s.security_opt.includes('no-new-privileges:true')||(s.secrets||[]).length!==0||s.ports)throw Error('Local provider fixture hardening drift');}
