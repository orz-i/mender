...
import { draftToolInput, type CatalogBindingInput, type CatalogPreflight, type CatalogToolVersion, type CatalogToolVersionInput } from '../domain/catalog';
...
export function CatalogManagementPage({ gateway }: { gateway: CatalogGateway }) {
  ...
  return <>
    ...
    <div className="page-heading-row"><div><h1>Catalog / Toolset</h1><p className="lead">创建与编辑 draft ToolVersion，绑定 Connection 与 Budget，再由服务端对持久化 Catalog、Pricing、Connection、Budget facts 做发布前校验。已发布版本不可原地修改。</p></div></div>
    <div className="success-panel" role="note"><strong>发布资格由服务端裁决</strong><span>页面不会根据当前选项自行推断权限或可发布性；Preflight 只是服务端快照，Publish 会再次原子校验。</span></div>
    ...
    {workspaces.data && workspaces.data.length > 0 && <>
      ...
      {snapshot.data && <>
        ...
        <section className="run-list-panel" aria-label="ToolVersion lifecycle"><div className="panel-heading"><div><p className="section-kicker">Versions</p><h2>ToolVersion 生命周期</h2></div></div>
          {tools.length === 0 ? <div className="empty-state"><strong>当前 Workspace 没有管理中的 ToolVersion</strong></div> : <div className="run-table-wrap"><table className="run-table"><thead><tr><th>ToolVersion</th><th>Provider / Price</th><th>状态</th><th>更新时间</th><th>操作</th></tr></thead><tbody>{tools.map((tool) => { const pf = preflights[`tool:${tool.toolVersionId}`]; return <tr key={tool.toolVersionId}><td><strong>{tool.title}</strong><div className="mono">{tool.toolId}@{tool.version}</div><div className="muted-copy">{tool.toolVersionId}</div></td><td><div className="mono">{tool.providerId}</div><div className="mono">{tool.priceVersionId}</div></td><td><span className={`state-badge state-${tool.state}`}>{stateLabel[tool.state]}</span></td><td>{formatTime(tool.updatedAt)}</td><td><div className="credential-actions">{tool.state === 'draft' && <><Button type="button" variant="outline" onClick={() => { setEditingTool(tool.toolVersionId); setToolForm(formFromTool(tool)); }}>编辑</Button><Button type="button" variant="outline" disabled={busy !== ''} onClick={() => void preflightTool(tool.toolVersionId)}>Preflight</Button><Button type="button" disabled={!pf?.ready || busy !== ''} onClick={() => void act(`publish-tool:${tool.toolVersionId}`, async () => { await gateway.publishToolVersion(workspaceId, tool.toolVersionId); clearPreflight(`tool:${tool.toolVersionId}`); }, 'ToolVersion 已发布为 immutable version。')}>发布</Button></>}{tool.state === 'published' && <Button type="button" variant="outline" disabled={busy !== ''} onClick={() => void act(`retire-tool:${tool.toolVersionId}`, async () => { await gateway.retireToolVersion(workspaceId, tool.toolVersionId); }, 'ToolVersion 已退役。')}>退役</Button>}{tool.state === 'retired' && <span className="muted-copy">只读</span>}</div>{pf && <PreflightResult value={pf} />}</td></tr>; })}</tbody></table></div>}
        </section>
        ...
        <section className="run-list-panel" aria-label="Toolset lifecycle"><div className="panel-heading"><div><p className="section-kicker">Toolsets</p><h2>Toolset 生命周期</h2></div></div>
          {toolsets.length === 0 ? <div className="empty-state"><strong>当前 Workspace 没有 Toolset</strong></div> : toolsets.map((set) => { const pf = preflights[`set:${set.id}`]; return <article className="launch-card" key={set.id}><div className="panel-heading"><div><p className="section-kicker">{stateLabel[set.state]}</p><h3 className="mono">{set.id}</h3><p className="muted-copy">Published {formatTime(set.publishedAt)} · Retired {formatTime(set.retiredAt)}</p></div><div className="credential-actions">{set.state === 'draft' && <><Button type="button" variant="outline" disabled={busy !== ''} onClick={() => void preflightToolset(set.id)}>Preflight</Button><Button type="button" disabled={!pf?.ready || busy !== ''} onClick={() => void act(`publish-set:${set.id}`, async () => { await gateway.publishToolset(workspaceId, set.id); clearPreflight(`set:${set.id}`); }, 'Toolset 已发布。')}>发布</Button></>}{set.state === 'published' && <Button type="button" variant="outline" disabled={busy !== ''} onClick={() => void act(`retire-set:${set.id}`, async () => { await gateway.retireToolset(workspaceId, set.id); }, 'Toolset 已退役。')}>退役</Button>}</div></div>{pf && <PreflightResult value={pf} />}{set.bindings.length === 0 ? <div className="empty-state"><strong>尚无 Binding</strong></div> : <div className="run-table-wrap"><table className="run-table"><thead><tr><th>Tool</th><th>Connection</th><th>Budget</th><th>MCP</th><th>操作</th></tr></thead><tbody>{set.bindings.map((binding) => <tr key={binding.toolVersionId}><td><span className="mono">{binding.toolId}@{binding.toolVersionLabel}</span><div className="muted-copy">{binding.toolVersionId}</div></td><td className="mono">{binding.connectionId}</td><td className="mono">{binding.budgetId}</td><td>{binding.mcpExposed ? binding.mcpName : '不暴露'}</td><td>{set.state === 'draft' ? <Button type="button" variant="outline" disabled={busy !== ''} onClick={() => void act(`delete-binding:${set.id}:${binding.toolVersionId}`, async () => { await gateway.deleteBinding(workspaceId, set.id, binding.toolVersionId); clearPreflight(`set:${set.id}`); }, 'Binding draft 已删除。')}>删除</Button> : <span className="muted-copy">immutable</span>}</td></tr>)}</tbody></table></div>}</article>; })}
        </section>
      </>}
    </>}
  </>;
