...
本文件是 MCPX 的产品与系统设计基线，面向产品负责人、技术负责人、前后端工程师、测试和运维人员。目标是把“微内核＋插件化＋动态运行时”的方向转化为可评审的模块边界、数据合同、执行机制和验收标准。MCPX 在本文中作为项目工作名，不代表已经完成品牌或域名确认。
...
建议首发元工具为 catalog_search、catalog_inspect、run_start、run_get、run_cancel；需要交互输入时再开放 run_resume。这些名称是 MCPX 自定义的 Tool，不是新增 MCP 标准方法。
...
如果后续允许 Agent 回调 MCPX 工具，平台签发短时委托凭据，限定 root Run、工具集、有效期、步骤数和预算子分配。子任务消费从根预算分配，不再次全额冻结同一笔额度；计费叶子和编排附加费有独立业务键，防止父子重复扣费。
...
采用信封加密思路：数据加密密钥保护凭据，主密钥由秘密管理设施保护；轮换、解密和导出均审计。平台调用 token 与上游 token 分开，禁止把用户访问 MCPX 的令牌直接转给供应商。[S06]
...
详细周计划、依赖、验收和失败回退见《MCPX 分阶段实施与验收计划》，可修改的人日、状态、进度与风险见 Excel 台账。新增范围必须同时更新需求、ADR、WBS、测试和 Gate，不只在聊天记录中改变承诺。
