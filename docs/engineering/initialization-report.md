...
> **历史快照，不是当前状态入口。** 以下段落保留早期演进证据，其“当前”“待实现”只对当时版本成立。后续 Core Alpha、G3 和 S4 实现统一见[当前实施与验收状态](../planning/current-status.md)。不要继续向本历史文件追加“当前进度”。
...
上述“未开始”是早期台账处理方式，已由 2026-09-16 状态回填纠正：实现、已有自动化记录和正式验收分别记录；初始值保留在 `baseline_execution`。历史段落不能覆盖后续 G3 范围化决议，也不自动批准其他 Gate。
