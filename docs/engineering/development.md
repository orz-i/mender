...
前端 `/` 提供 Alpha 导航，`/status` 发送真实健康请求；`/workspaces` 在 Console OIDC 启用后读取 HttpOnly server-side session 与当前 Workspace membership；`/connections` 使用独立 Connection manager 角色列出安全元数据、允许 Owner/Admin 撤销，并可启动一个服务端 reviewed OAuth Provider 的 PKCE 创建流程；`/runs` 通过显式短时 Human → Run delegation 读取/取消 Execution。Run Explorer 已支持服务端 cursor/through-version Event 分页和显式选择后的 bounded Artifact JSON 内容预览。OIDC Cookie 本身不能访问 Run API；delegation 只包含 `run:read` 与可选 `run:cancel`，没有 `run:create`，token 只保存在页面内存且不会进入 Query key。Machine `/api/v1` 入口继续用于服务/Agent identity，Console 使用独立 `/api/console/v1` delegated surface。详见 [Console Run Explorer](2026-09-12-console-run-explorer.md)、[Console Run Results Alpha](2026-09-12-console-run-results-alpha.md)、[Workspace Console Alpha](2026-09-12-workspace-console-alpha.md)、[Connection self-service Alpha](2026-09-12-connection-self-service-alpha.md)、[Reviewed OAuth Connection Alpha](2026-09-12-connection-oauth-alpha.md) 与 [Human → Run delegation Alpha](2026-09-12-human-run-delegation-alpha.md)。
