...
前端 `/` 提供 Alpha 导航，`/status` 发送真实健康请求；`/workspaces` 在 Console OIDC 启用后读取 HttpOnly server-side session 与当前 Workspace membership；`/connections` 使用独立 Connection manager 角色列出安全元数据、允许 Owner/Admin 撤销，并可启动一个服务端 reviewed OAuth Provider 的 PKCE 创建流程；`/runs` 通过显式短时 Human → Run delegation 读取/取消 Execution。Run Explorer 已支持服务端 cursor/through-version Event 分页、bounded Artifact JSON 内容预览，以及来自持久 Commerce 事实的单 Run quota-cost 投影。`/usage` 使用 Browser Session + `usage:read` 和独立 Commerce observer DB role 展示 Budget window 与最近 reservation/settlement；所有额度通过 micro string 传输，前端以 BigInt 格式化，不把 quota settlement 描述成支付账务。OIDC Cookie 本身不能访问 Run API；delegation 只包含 `run:read` 与可选 `run:cancel`，没有 `run:create`，token 只保存在页面内存且不会进入 Query key。Machine `/api/v1` 入口继续用于服务/Agent identity，Console 使用独立 `/api/console/v1` delegated surface。详见 [Console Run Explorer](2026-09-12-console-run-explorer.md)、[Console Run Results Alpha](2026-09-12-console-run-results-alpha.md)、[Console Usage Observability Alpha](2026-09-12-console-usage-observability-alpha.md)、[Workspace Console Alpha](2026-09-12-workspace-console-alpha.md)、[Connection self-service Alpha](2026-09-12-connection-self-service-alpha.md)、[Reviewed OAuth Connection Alpha](2026-09-12-connection-oauth-alpha.md) 与 [Human → Run delegation Alpha](2026-09-12-human-run-delegation-alpha.md)。
...
Console `/launch` 将 launch discovery 与 Human StartRun delegation 组合成产品工作流：选择 Workspace → Tool/Connection → JSON arguments → caller charge cap → mint exact delegation → delegated Admission。若 HTTP 结果不确定，页面只在当前内存保留 prepared capability，并允许使用同一 token + Idempotency-Key 重试；目标和参数会保持锁定。成功后跳转/链接到 Run Explorer。详见 [Console Human StartRun Alpha](2026-09-12-console-human-start-run-alpha.md)。
