...
pnpm deploy build dist/releases/my-release
```
...
```powershell
pnpm deploy prepare deploy/local.example.json dist/releases/my-release .local/deploy/local
pnpm deploy preflight .local/deploy/local
pnpm deploy up .local/deploy/local --apply
pnpm deploy status .local/deploy/local
```
...
`pnpm deploy stop .local/deploy/local` 停止进程但保留卷。不要使用其他项目的容器名，也不要执行不加实例限制的 prune 或 down --volumes。部署生成目录中的 CA 私钥、密码和配置需备份并限制 ACL，不能上传 Git。
...
```powershell
pnpm deploy operator .local/deploy/local provision-human --user user_maker --display-name Maker --issuer https://YOUR_ISSUER --oidc-subject REAL_SUB --workspace ws_local --membership-role owner
pnpm deploy operator .local/deploy/local provision-platform-staff --user user_maker --platform-role operator
```
...
pnpm deploy preflight .local/deploy/production --signed .local/operator/release-envelope.json --trusted-key .local/operator/release-public.pem
pnpm deploy up .local/deploy/production --apply --signed .local/operator/release-envelope.json --trusted-key .local/operator/release-public.pem
```
...
```powershell
pnpm deploy backup .local/deploy/local .local/backups/before-update --apply
pnpm deploy stop .local/deploy/local
pnpm deploy switch-release .local/deploy/local dist/releases/reviewed-next --apply --migrations-reviewed
pnpm deploy preflight .local/deploy/local
pnpm deploy up .local/deploy/local --apply
```
